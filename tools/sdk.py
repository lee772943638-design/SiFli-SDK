...
#
# You don't have to use sdk.py, you can use cmake directly
...
def check_environment() -> List:
    ...
    if 'SIFLI_SDK_PATH' in os.environ:
        ...
        if set_sifli_sdk_path != detected_sifli_sdk_path:
            print_warning(
                'WARNING: SIFLI_SDK_PATH environment variable is set to %s but %s path indicates SIFLI SDK directory %s. '
    ...
    try:
        ...
        # here as well (for example one can call sdk.py from a not properly set-up environment).
    ...
    try:
        ...
        if python_venv_path and not sys.executable.startswith(python_venv_path):
            print_warning(f'WARNING: Python interpreter "{sys.executable}" used to start sdk.py is not from installed venv "{python_venv_path}"')
    except KeyError:
...
def init_cli(verbose_output: Optional[List]=None) -> Any:
    ...
    class CLI(click.Group):
        ...
        def shell_complete(self, ctx: click.core.Context, incomplete: str) -> List[CompletionItem]:
            ...
            # unreliably in bash.
            complete_file = ('bash' not in os.environ.get('_SDK.PY_COMPLETE', '') or
                             '@' not in os.environ.get('SDK_PY_COMP_WORDBREAKS', ''))
        ...
        def execute_tasks(self, tasks: List, **kwargs: str) -> OrderedDict:
            ...
            # Execute all global action callback - first from sdk.py itself, then from extensions
            ...
            # when global_args.dry_run is true sdk.py works in idle mode and skips actual task execution
    ...
    project_dir = parse_project_dir(standalone_mode=False, complete_var='_SDK.PY_COMPLETE_NOT_EXISTING')

    all_actions: Dict = {}
    # Load extensions from components dir
    sdk_py_extensions_path = os.path.join(os.environ['SIFLI_SDK_PATH'], 'tools', 'sdk_py_actions')
    extension_dirs = [os.path.realpath(sdk_py_extensions_path)]
    ...
    for directory in extension_dirs:
        if directory and not os.path.exists(directory):
            print_warning('WARNING: Directory with sdk.py extensions doesn\'t exist:\n    %s' % directory)
            continue
    ...
    # Load component manager sdk.py extensions if not explicitly disabled
    # TODO:
    # if os.getenv('SIFLI_SDK_COMPONENT_MANAGER') != '0':
    #     extensions.append(('component_manager_ext', sdk_extensions))
    ...
    for name, extension in extensions:
        try:
            ...
            print_warning('WARNING: Cannot load ask.py extension "%s"' % name)

    # Load extensions from project dir
    if os.path.exists(os.path.join(project_dir, 'sdk_ext.py')):
        sys.path.append(project_dir)
        try:
            from sdk_ext import action_extensions
        except ImportError:
            print_warning('Error importing extension file sdk_ext.py. Skipping.')
            print_warning(
    ...
    cli_help = (
        'SiFli-SDK CLI build management tool. '
        'For commands that are not known to sdk.py an attempt to execute it as a build system target will be made. ')

    return CLI(help=cli_help, verbose_output=verbose_output, all_actions=all_actions)


def main(argv: Optional[List[Any]] = None) -> None:
    # Check the environment only when sdk.py is invoked regularly from command line.
...
def expand_file_arguments(argv: List[Any]) -> List[Any]:
    ...
    if expanded:
        print(f'Running: sdk.py {" ".join(argv)}')
