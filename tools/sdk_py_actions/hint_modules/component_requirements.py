...
from sdk_py_actions.tools import get_build_context
...
def generate_hint(output: str) -> Optional[str]:
    ...
    if not found_dep_component_names:
        ...
        if candidate_component_include_dirs:
            ...
            return (f'Missing "{missing_header}" file name found in the following component(s): {candidates}. '
                    ...
                    f'of sdk_component_register call in CMakeLists.txt. Another possibility may be that the '
                    f'component or its feature is not enabled in the configuration. Use "sdk.py menuconfig" '
    ...
    if source_component_has_priv_dep:
        problem_solution = (
            ...
            f'REQUIRES list of sdk_component_register call in {cmakelists_file_to_fix}.')
    else:
        problem_solution = (
            ...
            f'of sdk_component_register call in {cmakelists_file_to_fix}.')
