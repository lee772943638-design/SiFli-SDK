...
from sdk_py_actions.errors import NoSerialPortFoundError

from .constants import GENERATORS
from .errors import FatalError

# Name of the program, normally 'sdk.py'.
# Can be overridden from sdk.bat using SDK_PY_PROGRAM_NAME
PROG = os.getenv('SDK_PY_PROGRAM_NAME', 'sdk.py')

# environment variable used during click shell completion run
SHELL_COMPLETE_VAR = '_SDK.PY_COMPLETE'
...
def _sdk_version_from_txt() -> Optional[str]:
    """Acquires version of SiFli-SDK from version.txt"""
    version_path = os.path.join(os.environ['SIFLI_SDK_PATH'], 'version.txt')
    try:
        with open(version_path, encoding='utf-8') as f:
            ...
            if version:
                ...
                sys.stderr.write('WARNING: version.txt is empty\n')
                return None
    except OSError:
        sys.stderr.write('WARNING: Cannot find SiFli-SDK version in version.txt\n')
        return None

def sdk_version() -> Optional[str]:
    """Print version of SiFli-SDK"""

    #  Try to get version from git:
    try:
        version: Optional[str] = subprocess.check_output([
            'git',
            '--git-dir=%s' % os.path.join(os.environ['SIFLI_SDK_PATH'], '.git'),
            '--work-tree=%s' % os.environ['SIFLI_SDK_PATH'],
...
def debug_print_sdk_version() -> None:
    print_warning(f'SiFli-SDK {sdk_version() or "version unknown"}')
...
def generate_hints_buffer(output: str, hints: Dict) -> Generator:
    ...
    # get the output unchanged. Please see tools/sdk_py_actions/hints.yml
...
class RunTool:
    ...
    async def run_command(self, cmd: List, env_copy: Dict) -> Tuple[Process, Optional[str], Optional[str]]:
        ...
        try:
            ...
            pass
        # Note: we explicitly pass in os.environ here, as we may have set SIFLI_SDK_PATH there during startup
        # limit was added for avoiding error in sdk.py confserver
        try:
            ...
            message = f'ERROR: {sys.executable} doesn\'t support asyncio. The issue can be worked around by re-running sdk.py with the "--no-hints" argument.'
        ...
        stderr_output_file = os.path.join(self.build_dir, log_dir_name, f'sdk_py_stderr_output_{p.pid}')
        stdout_output_file = os.path.join(self.build_dir, log_dir_name, f'sdk_py_stdout_output_{p.pid}')
    ...
    async def read_and_write_stream(self, input_stream: asyncio.StreamReader, output_filename: str,
        ...
        output_converter = output_stream
        ...
        try:
            ...
            with open(output_filename, 'w', encoding='utf8', newline='') as output_file:
                ...
                while True:
                    ...
                    # If sdk.py output is redirected and the output stream is not a TTY,
...
def run_tool(*args: Any, **kwargs: Any) -> None:
    # Added in case someone uses run_tool externally in sdk.py extensions
...
def run_target(target_name: str, args: 'PropertyDict', env: Optional[Dict]=None,
    ...
    # If sdk.py's stdout is not redirected, the final output is a TTY, so we can tell Make/Ninja to disable stripping
...
def _check_sdk_target(args: 'PropertyDict', prog_name: str, cache: Dict,
