                                      ...
                                      ustorage_cbw_t cmd, rt_uint8_t *buffer, int timeout)
{
    ...
    {
        /* send the cbw */
        //rt_kprintf("%s %d cmd=%p,buffer=%p\n",__func__,__LINE__,cmd,buffer);
        size = rt_usb_hcd_pipe_xfer(stor->pipe_out->inst->hcd, stor->pipe_out,
                                    cmd, SIZEOF_CBW, timeout);
        if (size != SIZEOF_CBW)
        {
            rt_kprintf("CBW size error,size=%d\n", size);
            return -RT_EIO;
        }
        if (cmd->xfer_len != 0)
        {
        ...
        size = rt_usb_hcd_pipe_xfer(stor->pipe_in->inst->hcd, stor->pipe_in,
                                    (void *)&csw, SIZEOF_CSW, timeout);
        if (size != SIZEOF_CSW)
        {
            rt_kprintf("csw size error size=%d\n", size);
            return -RT_EIO;
        }
    }
    while (0);
    ...
    {
        rt_kprintf("csw signature error,csw.signature=0x%x,csw.tag=0x%x\n", csw.signature, csw.tag);
        return -RT_EIO;
    }
...
{
    ALIGN(32) struct ustorage_cbw cmd;
    int timeout = USB_TIMEOUT_LONG;
