...
rt_err_t rt_udisk_run(struct uhintf *intf)
{
    ...
    char sname[8];
    char udisk_path[8];
    ALIGN(32) rt_uint8_t max_lun, *sector, sense[18], inquiry[36];
...
//    if (ret != RT_EOK) return ret;
    ...
    for (i = 0; i < MAX_PARTITION_COUNT; i++)
    {
        ...
        if (ret == RT_EOK)
        {
            ...
            rt_snprintf(udisk_path, 8, "/udisk%d", data->udisk_id);
            ...
            mkdir(udisk_path, 0);
            if (dfs_mount(stor->dev[i].parent.name, udisk_path, "elm",
                          0, 0) == 0)
            {
        ...
        else
        {
            if (i == 0)
            {
                ...
                if (dfs_mount(stor->dev[0].parent.name, UDISK_MOUNTPOINT, "elm", 0, 0) == 0)
                {
                    rt_kprintf("Mount FAT on Udisk successful.name=%s\n", stor->dev[0].parent.name);
                }
                else
                {
                    rt_kprintf("Mount FAT on Udisk failed.ret=%d\n", ret);
                    ret = -RT_ERROR;
                }
            }
