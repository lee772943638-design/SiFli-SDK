...
{
    rt_kprintf("bt_app_interface_event_handle type %d, event_id %d\n", type, event_id);
...
#else
    static const char *local_name = "sifli_streaming";
#endif
...
void bt_app_entry(void *param)
{
    ...
    int ret = bt_interface_register_bt_event_notify_callback(bt_app_interface_event_handle);

    rt_kprintf("bt_interface_register_bt_event_notify_callback return: %d\n", ret);
    ...
    while (1)
    {
        ...
        if (RT_EOK == rt_mb_recv(g_bt_app_mb, (rt_uint32_t *)&value, RT_WAITING_FOREVER) && value == BT_APP_READY)
...
void bt_app_init(void)
{
    ...
    tid = rt_thread_create("bt_app", bt_app_entry, NULL, 2048, RT_THREAD_PRIORITY_MIDDLE - 1, RT_THREAD_TICK_DEFAULT);
